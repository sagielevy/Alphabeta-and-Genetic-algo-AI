...
std::vector<Cell> Game::toArr(const Save& save) const
{
	using Bit::Masks::S;
...
void Game::print(const Save& save) const
{
	using std::cout;
	using std::endl;
	...
	if (save.turn)
		...
		cout << "Player 2's turn.\n";
	cout << "P1: " << Bit::bitCount(save.BP) << "\tP2: " << Bit::bitCount(save.WP)
		<< endl;

	std::vector<Cell> b = toArr(save);
...
BitBoard Game::getJumpers(const Save& save) const
{
	using namespace Bit::Masks;
	using Bit::ror;
	using Bit::rol;

	BitBoard empty = ~(save.WP | save.BP);
	BitBoard Temp;
	BitBoard jumpers = 0;
	if (save.turn) {
		BitBoard BK = save.BP & save.K;
		Temp = ror(empty, 7) & save.WP & CAN_UPLEFT;
		jumpers |= ror(Temp, 7) & save.BP & CAN_UPLEFT;
		Temp = ror(empty, 1) & save.WP & CAN_UPRIGHT;
		jumpers |= ror(Temp, 1) & save.BP & CAN_UPRIGHT;

		Temp = rol(empty, 7) & save.WP & CAN_DOWNRIGHT;
		jumpers |= rol(Temp, 7) & BK & CAN_DOWNRIGHT;
		Temp = rol(empty, 1) & save.WP & CAN_DOWNLEFT;
		jumpers |= rol(Temp, 1) & BK & CAN_DOWNLEFT;
	} else {
		BitBoard WK = save.WP & save.K;
		Temp = rol(empty, 7) & save.BP & CAN_DOWNRIGHT;
		jumpers |= rol(Temp, 7) & save.WP & CAN_DOWNRIGHT;
		Temp = rol(empty, 1) & save.BP & CAN_DOWNLEFT;
		jumpers |= rol(Temp, 1) & save.WP & CAN_DOWNLEFT;

		Temp = ror(empty, 7) & save.BP & CAN_UPLEFT;
		jumpers |= ror(Temp, 7) & WK & CAN_UPLEFT;
		Temp = ror(empty, 1) & save.BP & CAN_UPRIGHT;
...
BitBoard Game::getMovers(const Save& save) const
{
	using namespace Bit::Masks;
	using Bit::ror;
	using Bit::rol;

	const BB empty = ~(save.WP | save.BP);
	BB Movers;

	if (save.turn) {
		const BB BK = save.BP & save.K;
		Movers = ror(empty, 7) & save.BP & CAN_UPLEFT;
		Movers |= ror(empty, 1) & save.BP & CAN_UPRIGHT;
		Movers |= rol(empty, 7) & BK & CAN_DOWNRIGHT;
		Movers |= rol(empty, 1) & BK & CAN_DOWNLEFT;
	} else {
		const BB WK = save.WP & save.K; // Kings
		Movers = rol(empty, 7) & save.WP & CAN_DOWNRIGHT;
		Movers |= rol(empty, 1) & save.WP & CAN_DOWNLEFT;
...
BB Game::canJump(const BB src, const BB vict) const
{
	using Bit::rol;
	using Bit::ror;
	using namespace Bit::Masks;
...
BB Game::canJump(const Save& save, const BB src, const BB vict) const
{
	using Bit::rol;
	using Bit::ror;
	using namespace Bit::Masks;
	...
	if (save.mustJump)
		if (src != save.mustJump)
			return 0u;
	BB Temp;
	BB SK;
	BB empty = ~(save.WP | save.BP);
...
{
	// Basic grading system. This'll do for non-AI checking of board status
